 ...
 * М8О-201Б-18
 * Вариант 15:
 * Создать класс TransNumber для работы с трансцендентными числами. Трансцендентное число представлено парой
 * (a,b), где a - действительная часть, b - трансцендентная часть. Трансцедентная часть
 * представляет собой действительное число b, умноженное на константу sqrt(2). Реализовать арифметические операции
 * (по аналогии с операциями над комплеклснымии числами в алгебраической форме), и операции сравнения по значению
 * (a + b*sqrt(2)).
 */
#include<iostream>
#include<cmath> // для операции сравнения по значению
#include<stdexcept>
using namespace std;
...
public:
    ...
    TransNumber(); // Конструктор для инициализации
    ...
    const char * equ(TransNumber const rhs); // объявление функций для операций над числами
    void write(std::ostream &out);
    void read(std::istream &in);
    friend void record(TransNumber &lhs, TransNumber &rhs);
}; // описание класса для работы с трансцедентными числами
...
void record(TransNumber &lhs, TransNumber &rhs) {
    ...
    rhs.read(std::cin);
}//функция ввода двух чисел вида (a,b)
